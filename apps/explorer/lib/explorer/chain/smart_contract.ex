defmodule Explorer.Chain.SmartContract do
  ...
  * `verified_via_eth_bytecode_db` - whether contract automatically verified via eth-bytecode-db or not.
  ...
  """
  @spec proxy_contract?(__MODULE__.t(), Keyword.t()) :: boolean()
    ...
    implementation_method_abi =
      ...
      |> Enum.find(fn method ->
        Map.get(method, "name") == "implementation" ||
          Proxy.master_copy_pattern?(method)
      end)
  ...
  @doc """
  Returns implementation address of the given SmartContract by hash address
  """
  @spec get_implementation_address_hash(list(), Keyword.t()) :: {nil, nil} | {Hash.Address.t(), String.t()}
  ...
  """
  @spec select_partially_verified_by_address_hash(binary() | Hash.t(), keyword) :: __MODULE__.t() | nil
  ...
  def compose_smart_contract(address_result, hash, options) do
    address_verified_twin_contract = get_address_eip1167_or_verified_twin_contract(hash, options)
  ...
  def get_address_verified_twin_contract(hash, options) when is_binary(hash) do
    case Chain.string_to_address_hash(hash) do
      {:ok, address_hash} -> get_address_eip1167_or_verified_twin_contract(address_hash, options)
  ...
  @spec get_address_eip1167_or_verified_twin_contract(Hash.Address.t(), Keyword.t()) :: Hash.Address.t()
  def get_address_eip1167_or_verified_twin_contract(address_hash, options \\ []) do
  ...
  def address_hash_to_smart_contract(address_hash, options \\ []) do
    ...
    current_smart_contract = Chain.select_repo(options).one(query)

    if current_smart_contract do
      current_smart_contract
    else
      address_verified_twin_contract = get_address_eip1167_or_verified_twin_contract(address_hash, options)
  ...
  defp check_verified_with_full_match(address_hash, options) do
    ...
    result = Chain.select_repo(options).one(query)
  ...
  defp address_hash_to_smart_contract_without_twin(address_hash, options) do
    query = get_smart_contract_query(address_hash)
